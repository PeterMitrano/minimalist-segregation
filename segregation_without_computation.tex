...
%\author{
  ...
  %mjgiancola@wpi.edu}
  %\and
  %\IEEEauthorblockN{Carlo Pinciroli}
  %\IEEEauthorblockA{\textit{Worcester Polytechnic Institute} \\
  %Worcester, MA \\
  %cpinciroli@wpi.edu}
%}
...
\section{Controller Analysis}
  ...
  \subsection{Aggregation with Static Kin}
    ...
    \begin{figure}
      ...
      \begin{tikzpicture}[scale=25.0]
        ...
        \node at (-.02,0.1) {y};
         ...
         % c_i
        \filldraw (0,0) circle (.003);
        \node at (.02,.02) {$c_i$};
        \draw[blue, dashed] (0,0) circle (0.053);

         % p_i
        \filldraw (0,-0.053) circle (.003);
        \node at (0,-0.065) {$p_i$};
        ...
        \node at (-0.015,-0.061) {$r_i$};
         ...
         % p'_i
        \filldraw (0.0131,-0.051) circle (.003);
        \node at (0.019,-0.038) {$p'_i$};
        \draw[blue] (0.0131,-0.051) circle (0.035);
        \draw[dashed] (0,0) -- (0.0131,-0.051);

         % p_j
        \filldraw (0.14,-0.277) circle (.003);
        \node at (0.16,-0.14) {$p_j$};
        \draw[red] (0.14,-0.277) circle (0.224);
        \draw[dashed] (.14,-0.277) -- (.14,-.053);
        \node at (.15,-0.075) {$r_j$};

        % delta
        \draw[blue] (0,-.053) -- (.14,-.053);
        \node at (0.075, -0.06) {$\delta$};
    ...
    With this coordinate system defined, we can define the coordinates of $p_i$, $p_j$, and $p'_i$ (Equation \ref{eq:vars}). Next, we can write the variables $\theta$ and $R$ in terms of $\Delta t$, the inter wheel diameter $W$, and the wheels speeds of the controller $v_{l_1}$ and $v_{r_1}$, which are shown in Equation \ref{eq:theta_and_r}.
    ...
    Equation \ref{eq:agg} is useful because it tells us whether aggregation is guaranteed knowing only the sizes of our robots. Of course this condition may not hold true for all scenarios, but we show it holds true for many popular robots in Table \ref{table:robots}). This table shows that largest $r_j$ for which aggregation is guaranteed.
    ...
    \begin{table}
      ...
      \begin{tabular}{|c|c|c|c|} \hline
        ...
        Kilobot\footnote{\href{https://www.k-team.com/mobile-robotics-products/kilobot/specifications}{https://www.k-team.com/mobile-robotics-products/kilobot/specifications}} &
            0.0165 & 0.030\footnote{only approximate} & 0.043 \\ \hline
      \end{tabular}
      \label{table:robots}
    \end{table}
    \end{savenotes}
  ...
  \subsection{Proof of Theorem 1} \label{thm:1}
    ...
    {%
      ...
      \begin{align*}
        ...
        (R+r_j)\tan{\frac{\theta}{2}}  &< \sqrt{d^2 - r^2_j} \\
        \bigg((R+r_j)\tan{\frac{\theta}{2}}\bigg)^2 &< d^2 - r^2_j \\
        \bigg((R+r_j)\tan{\frac{\theta}{2}}\bigg)^2 + r^2_j &< d^2 \\
        \intertext{Aggregation is achieved when $d^2 \leq r^2_i + r^2_j$, so we can simplify our condition for guaranteed aggregation to:}
        \bigg((R+r_j)\tan{\frac{\theta}{2}}\bigg)^2 + r^2_j &< r^2_i + r^2_j \\
        \bigg((R+r_j)\tan{\frac{\theta}{2}}\bigg)^2 &< r^2_i \\
        (R+r_j)\tan{\frac{\theta}{2}} &< r_i \qquad \text{assuming positive terms} \\
        (W+r_j)\tan{\frac{\Delta t}{15W}} &< r_i \\
        r_j &< \frac{r_i}{\tan{\frac{\Delta t}{15W}}} - W \\
      \end{align*}
    }
