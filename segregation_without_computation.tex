...
\section{Controller Analysis}
  ...
  \subsection{Aggregation with Static Kin}
    ...
    \begin{figure}
      ...
      \begin{tikzpicture}[scale=22.0]
        ...
        \draw[dashed] (0,0) -- (0.0131,-0.051);
         ...
         % p_j
        \filldraw (0.22, -0.153) circle (.003);
        \node at (0.22, -0.13) {$p_j$};
        \draw[red] (0.22, -0.153) circle (0.1);
        \draw[dashed] (0.12, -0.153) -- (0.22, -0.153);
        \node at (0.17, -0.17) {$r_j$};

        % delta
        \draw[blue] (0,-.053) -- (.22,-.053);
        \node at (0.1, -0.068) {$\delta$};
    ...
    Equation \ref{eq:agg} is useful because it tells us whether aggregation is guaranteed knowing only the sizes of our robots or the largest cluster we expect to form. This condition may not hold true for all scenarios (such as those with thousands of small robots), but we show it holds true for many popular robots in Table \ref{table:robots}). This table shows that largest cluster size $r_j$ for which aggregation is guaranteed, assuming a controller period of \SI{0.1}{\second}.
    ...
    \begin{table}
      ...
      \begin{tabular}{|c|c|c|c|} \hline
        ...
        Kilobot\footnote{\href{https://www.k-team.com/mobile-robotics-products/kilobot/specifications}{https://www.k-team.com/mobile-robotics-products/kilobot/specifications}} &
            0.0165 & 0.030\footnote{only approximate} & - \\ \hline
      \end{tabular}
      \label{table:robots}
    \end{table}
    \end{savenotes}
