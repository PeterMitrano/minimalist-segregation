...
\section{The Emergent Behavior}
    ...
    After running 10 generations with 10 genomes per generation, the lowest cost controller we found was [0.5, -0.5, -0.2, 0.6, 0.6, 0.2]. Similarly, after running the grid search the best controller was [1, -$\sfrac{2}{3}$, $\sfrac{1}{3}$, 1, 1, 0]. For both of these controllers, the resulting behavior is to turn away from kin but turn the opposite way when they see nothing or non-kin. This cause the kin robots to zig-zag in a line towards their kin, and when multiple robots execute this behavior the kin robots form rings. An example of this can be seen below in Figure \ref{fig:rings}. Rarely, they also for these shrimp-like shapes which can also be seen in Figure \ref{fig:rings}.
    ...
    \begin{figure}
      ...
      \begin{tikzpicture}[scale=25.0]
        ...
        \draw[dashed] (0,0) -- (0.0131,-0.051);
         ...
         % p_j
        \filldraw (0.15, -0.128) circle (.003);
        \node at (0.15, -0.11) {$p_j$};
        \draw[red] (0.15, -0.128) circle (0.075);
        \draw[dashed] (0.075, -0.128) -- (0.15, -0.128);
        \node at (0.11, -0.137) {$r_j$};
        ...
        % d
        \draw (0,-0.053) -- (0.15, -0.128);
        \node at (0.06,-0.09) {$d$};
      \end{tikzpicture}
      \caption{The generic configuration of a robot aggregating to another kin or cluser of kin.}
