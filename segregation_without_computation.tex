...
\author{
  \IEEEauthorblockN{%
    ...
    Jordan~Burkland,
    Michael~Giancola,
    Carlo~Pinciroli\IEEEmembership{Member,~IEEE}}
  \IEEEauthorblockA{Robotics Engineering, Worcester Polytechnic Institute, Worcester, MA, USA\\
...
\begin{algorithm}[t]
  \begin{algorithmic}
    \If {$S=0$} \State set wheel speeds to $v_{l_0}$, $v_{r_0}$
    \ElsIf {$S=1$} \State set wheel speeds to $v_{l_1}$, $v_{r_1}$
...
$i \ne j, \epsilon \in \mathbb{R}^+$. In our experiments, we set
$\epsilon = \SI{5}{\centi\meter}$.  A \emph{cluster} can be intuitively defined
as an `island' of connected robots, and it can be found with a depth-first
search across the robots. Since we are interested in segregating the robots in
$N$ classes, the final result of a trial is expected to be a set of $N$ distinct
clusters \emph{composed of kin robots}. Hence, we define a cluster as an island
of connected kin robots.
...
$c_{\text{total}}$ correctly assigns cost in most scenarios, thus fitting well
our analysis purposes. However, this cost function considers a straight line of
robots to be a cluster, and as such it might not be ideal for scenarios in which
the clusters are required to be tight.
