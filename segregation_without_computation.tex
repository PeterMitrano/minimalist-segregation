...
\section{Controller Analysis}
  ...
  \subsection{Aggregation of Two Kin}
    ...
    We need to show that, in transitions $y$ and $z$, the kin robots always aggregate more than $s$. We first demonstrate that the magnitude of aggregation, which can be described as $d'-d$ in Figure \ref{fig:kin_aggregation}, always increases as $\delta$ increases. In other words, the closer the robots are, the less by which they aggregate. The conditions under which this is true are derived in Theorem\ref{thm:decrease}. Given this fact, if we can then also prove that the robots aggregate by more than $s$ at the smallest $\delta$, then they must also aggregate for all $\delta$. This second condition is derived in Theorem \ref{thm:one_see_cond} and \ref{thm:both_see_cond}, and the final conditions are shown below. We know the lower bound of $\delta$ is $\sqrt{2}r$ because this is the point at which the two robots are touching.
  ...
  \subsection{Segregation}
    ...
    So far all of the behaviors we have discussed are actually just aggregation behaviors. Here we partially explain why segregation occurs by showing that between two robots $i$ and $j$, the kin robots aggregate faster than the non-kin robots.
...
\section{Appendix}
  ...
  \begin{proof}
    ...
    If the conditions in Theorem \ref{thm:decrease} hold, then we can again say that the distance by which the robots aggregate decreases as $\delta$ decreases. Therefore, if we show that aggregation is guaranteed for the minimum $\delta=\sqrt{r^2+r_j^2}$, then it is also guaranteed for all greater $\delta$.
    ...
    \begin{align*}
      ...
      \sqrt{(\sqrt{2}r_j - 2W\sin(\theta_i))^2 + (-r_j-2W+2W\cos(\theta_i))^2} - \sqrt{3}r_j &< 0
    \end{align*}

    Again, If we have already shown that Theorem \ref{thm:one_see_cond} or \ref{thm:both_see_cond} are true then the above condition must also hold true because $s>0$. Since the condition for kin-to-ring aggregation is less strict, it may be guaranteed even if two-kin aggregation is not guaranteed.
