...
\section{Controller Analysis}
  ...
  \subsection{Aggregation of Two Kin}
    ...
    Note that this proof relies on the assumption that the robots cannot transition back to $0,0$ forever, which is true because in this state they rotate and without any occlusions they must see each other at some point before completing a full rotation. In practice, this even less of an issue because the robots almost never have to turn more than half way around before detecting another robot.

    Ultimately, if Equations \eqref{eq:two_kin_agg_cond_1}, \eqref{eq:two_kin_agg_cond_2}, and \eqref{eq:two_kin_agg_cond_3} are satisfied, we can say that form any initial configuration, two kin robots will eventually aggregate in a sort of two-steps-forward one-step-back fashion. This behavior is clearly demonstrated in our simulation and real robot experiments.
			...
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm, semithick]
				...
				\path (A) edge              node {} (B)
							...
							(B) edge [loop above] node {} (B)
									edge [bend left=15]  node {} (C)
							(C) edge [bend left=15]  node {} (A)
									edge [bend left=15]  node {} (B);
			\end{tikzpicture}
      \label{fig:fsm}
      \caption{State machine representation for two kin aggregation. (1,0) means robot $i$ has sensor state $I=0$, and $j$ has $I=1$}
		\end{figure}
