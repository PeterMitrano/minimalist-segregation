...
of why certain behavior is guaranteed for the best parameter choice found; and
...
$\vec{p}_i(t) = [x_i(t)\;y_i(t)]$ as the $x$ and $y$ coordinates of the robot $i$ at time $t$, we consider any two kin robots to be
connected if
...
should grow (and maintain) clusters over time. In our experiments, we found from visual inspection that
$c_{\text{total}}$ correctly assigns cost in most scenarios.
However, this cost function considers a straight line of robots to be a cluster,
and as such it might not be ideal for scenarios in which the clusters are required to be tight.
...
\begin{table}[b]
  ...
  \begin{tabular}{|c|c|c|c|} \hline
    ...
    $\omega$ & $\frac{4V}{3l}$ & $\frac{2V}{3l}$ & $\frac{2V}{l}$ \\ \hline
  \end{tabular}
  
  \caption{turning Radii and rotational velocities for the best parameters.\label{tab:omega_and_r}}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=1\linewidth]{./images/beam_length}
