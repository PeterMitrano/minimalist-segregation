...
\begin{theorem}[Kin Robots \#1]
  ...
  \begin{equation}\label{eq:two_nothing_condition}
    V\Delta t \leq 3l\tan^{-1}\Bigg(\frac{\sqrt{3}r}{r-2l}\Bigg)
  \end{equation}
\end{theorem}
\begin{proof}
  To find a condition underwhich $c_i$ moves towards $c_j$ for any configuration $(x,y,\alpha)$, we first find the configuration which gives the tighted bound on the angle $\bar{\theta}$. 
\end{proof}
